#ifndef IOTYK_CERTIFICATES_H
#define IOTYK_CERTIFICATES_H

#include <string.h>

/*
 * ===========================================================================
 * SECTION A: EMQX MQTT CA — paste manually following these rules:
 * ===========================================================================
 * 1. Log into EMQX Cloud console
 * 2. Go to: Deployment → Overview → Connection guide
 * 3. Download the CA certificate (emqxsl-ca.crt)
 * 4. Open in text editor, copy everything from -----BEGIN to END-----
 * 5. Paste below replacing the PASTE_EMQX_CA_CERTIFICATE_HERE placeholder
 *
 * Rules:
 *   - Keep -----BEGIN CERTIFICATE----- and -----END CERTIFICATE-----
 *   - No leading spaces on cert lines
 *   - No extra blank lines inside the cert block
 *   - The )EOF"; closing must stay on its own line
 *
 * Example of correct format after pasting:
 * static const char EMQX_MQTT_CA_CERT[] PROGMEM = R"EOF(
 * -----BEGIN CERTIFICATE-----
 * MII... (many base64 lines)
 * -----END CERTIFICATE-----
 * )EOF";
 * ===========================================================================
 */

static const char EMQX_MQTT_CA_CERT[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----
PASTE_EMQX_CA_CERTIFICATE_HERE
-----END CERTIFICATE-----
)EOF";

/*
 * ===========================================================================
 * SECTION B: Local WSS — auto-generated by generate_certs.py
 * ===========================================================================
 * Generated with: python3 generate_certs.py --device-id <DEVICE_ID> --out-dir ./
 *
 * Phone trust instructions:
 *   Android: adb push ca.crt /sdcard/iotyk_ca.crt
 *            Settings → Security → Install certificate → CA certificate
 *   iOS:     AirDrop or email ca.crt → tap install
 *            Settings → General → About → Certificate Trust Settings → Enable
 *
 * After trusting the CA:
 *   Set #define LOCAL_WSS_ENABLED true in config.h
 * ===========================================================================
 */

// LOCAL_WSS_CA_CERT — CA certificate for local WSS
static const char LOCAL_WSS_CA_CERT[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

// LOCAL_WSS_SERVER_CERT — Server certificate for local WSS (SAN: DNS:<device>.local, localhost, IP:127.0.0.1)
static const char LOCAL_WSS_SERVER_CERT[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

// ⚠ WARNING: Keep private key secret — do not commit to version control!
// LOCAL_WSS_PRIVATE_KEY — Private key for local WSS server
static const char LOCAL_WSS_PRIVATE_KEY[] PROGMEM = R"EOF(
-----BEGIN RSA PRIVATE KEY-----

VivcLFOjkoKt1jKZSmzxHa8UlBVDT4vkT9r07UvSDa24wcYWFS2C
-----END RSA PRIVATE KEY-----
)EOF";

// Helper function to check if a CA certificate is usable (not placeholder)
inline bool hasUsableCa(const char* ca) {
  return ca && strstr(ca, "PASTE_") == nullptr && strlen(ca) > 100;
}

// Helper function to check if a cert/key pair is usable (not placeholder)
inline bool hasUsableCertPair(const char* cert, const char* key) {
  return cert && key
    && strstr(cert, "PASTE_") == nullptr
    && strstr(key, "PASTE_") == nullptr
    && strlen(cert) > 100
    && strlen(key) > 100;
}

#endif
