...
class MqttBridge {
  ...
  connect() {
    if (!hasRealEnvValue('EMQX_BROKER_URL') || !hasRealEnvValue('MQTT_SERVICE_USER') || !hasRealEnvValue('MQTT_SERVICE_PASS')) {
      console.warn('Server MQTT credentials are not configured. Bridge will not start.');
      return;
    }
